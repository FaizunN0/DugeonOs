...
const judi = (s) => {
  ...
  return {
    body: `
      <p class="app-lead">DungeonSlots - mesin slot bawah tanah. 3 simbol sama = jackpot! (DevConsole punya trik rahasia: /slotgacor.)</p>
      <div class="slot-bal">Saldo: <b>${bal}</b> chip · Gold: <b>${s.stats.gold}</b>g · Best: <b>${s.slots.best || 0}</b></div>
      <div class="slot-machine">
        <div class="slot-reel" id="slot-r0">${SLOTS_SYM[0]}</div>
        <div class="slot-reel" id="slot-r1">${SLOTS_SYM[1]}</div>
        <div class="slot-reel" id="slot-r2">${SLOTS_SYM[2]}</div>
      </div>
      <div class="slot-msg" id="slot-msg">Top up dulu buat main.</div>
      <div class="slot-actions">
        <button class="action-btn" id="slot-topup">${icon("coin")}<span>Top Up 50g = 100 chip</span></button>
        <button class="action-btn slot-spin" id="slot-spin">${icon("bolt")}<span>Spin (10 chip)</span></button>
      </div>`,
    mount(screen, state, handlers) {
      ...
      screen.querySelector("#slot-spin").addEventListener("click", () => {
        Sound.tap();
        if ((getState().slots.balance || 0) < 10) { toast("Saldo habis. Top up dulu.", { ico: "coin", cls: "toast-bad" }); return; }
        let last = null;
        mutate(st => {
          if ((st.slots.balance || 0) < 10) { toast("Saldo habis. Top up dulu.", { ico: "coin", cls: "toast-bad" }); return; }
          st.slots.balance -= 10;
          let win, big;
          if (st.flags.slotGacor) { win = true; big = Math.random() < 0.5; }
          else {
            const r = st.slots.rig || 0; st.slots.rig = r + 1;
            if (r <= 3) { win = true; big = true; }            // awal menang banyak
            else if (r <= 7) { win = Math.random() < 0.55; big = false; } // naik turun
            else if (r <= 10) { win = false; }                 // kalah
            else if (r <= 12) { win = Math.random() < 0.35; big = false; } // kiat lagi dikit
            else { win = false; }                              // FULL KAALAH -> Rungkat
          }
          let syms;
          if (win) {
            const k = Math.floor(Math.random() * SLOTS_SYM.length);
            if (big) syms = [k, k, k];
            else { let b; do { b = Math.floor(Math.random() * SLOTS_SYM.length); } while (b === k); syms = [k, k, b]; }
          } else {
          ...
          st.slots._last = syms; st.slots._win = win; st.slots._big = big;
          last = syms;
        });
        if (!last) return;
        let n = 0; const spin = setInterval(() => {
          r0.textContent = SLOTS_SYM[Math.floor(Math.random() * SLOTS_SYM.length)];
          r1.textContent = SLOTS_SYM[Math.floor(Math.random() * SLOTS_SYM.length)];
          r2.textContent = SLOTS_SYM[Math.floor(Math.random() * SLOTS_SYM.length)];
          if (++n > 12) {
            clearInterval(spin);
            r0.textContent = SLOTS_SYM[last[0]]; r1.textContent = SLOTS_SYM[last[1]]; r2.textContent = SLOTS_SYM[last[2]];
...
function resolveSpin(screen, handlers) {
  const st = getState();
  const win = st.slots._win, big = st.slots._big;
  let pay = win ? (big ? 120 : 30) : 0;
  const msg = screen.querySelector("#slot-msg");
  mutate(s => {
    if (win) { s.slots.balance += pay; s.stats.gold += Math.floor(pay / 4); if (pay > (s.slots.best || 0)) s.slots.best = pay; }
  });
  if (win) { Sound.good(); msg.textContent = "MENANG +" + pay + " chip!"; msg.className = "slot-msg good"; }
  else { Sound.bad(); msg.textContent = "Boncos. Coba lagi, bos."; msg.className = "slot-msg bad"; }
